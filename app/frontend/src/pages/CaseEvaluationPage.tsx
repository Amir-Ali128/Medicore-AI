...
const CASE_REVIEW_SCOPE_KEY = 'medicore:caseEvaluationScope:v2';
const CASE_REVIEW_RESULT_KEY = 'medicore:caseEvaluationResult:v2';
const AI_ROW_EMOJIS = ['🩺', '🧪', '🩻', '🧠', '📋', '⚕️'];
...
export default function CaseEvaluationPage() {
  ...
  async function evaluateAllSources() {
    ...
    try {
      ...
      setStatus('✅ Birleşik değerlendirme tamamlandı ve saklandı.');
    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-10">
      <header className="px-1">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
          Klinik bilgiler, laboratuvar sonuçları ve radyoloji raporları birlikte değerlendirilir. Sistem önceden seçilmiş tek bir hastalığa özel skor üretmez.
        </p>
      </header>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-[#2f1768]">
                Yapay zekâ klinik değerlendirmesi
              </h2>
              <p className="mt-1 text-sm leading-6 text-violet-800/70">
                Hazır olan üç veri kaynağı aynı vaka bağlamında birlikte incelenir.
              </p>
            </div>
          </div>
          <button
        ...
        {aiItems.length > 0 ? (
          ...
          <div className="px-5 py-6">
            <div className="rounded-2xl border border-dashed border-violet-300 bg-white/65 p-5 text-sm leading-7 text-violet-900">
              Birleşik değerlendirme henüz çalıştırılmadı. Klinik kayıt, kan analizi ve radyoloji raporunun üçü de hazır olduğunda değerlendirmeyi başlatabilirsin.
            </div>
          </div>
        )}
      ...
      <section className="rounded-[26px] border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-lg font-black text-slate-950">
              Değerlendirmeye dahil edilen kayıtlar
            </h2>
          </div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          <SourceCard
            ...
            detail={
              clinicalReady
                ? 'Hasta tarafından girilen klinik bilgiler aktif vaka içinde hazır.'
          ...
          <SourceCard
            ...
            detail={
              laboratoryReady
                ? `${labResults.length} sonuç · ${labCounts.high} yüksek · ${labCounts.low} düşük · ${labCounts.review} kontrol.`
                : 'Laboratuvar PDF’i yüklenmeli veya sonuçlar eklenmelidir.'
            }
            link="/analysis/mock"
            linkLabel="Laboratuvarı aç"
          />
          <SourceCard
            emoji="🩻"
            ...
            detail={
              latestReport
                ? `${reports.length} kayıt · en güncel: ${latestReport.modality} · ${latestReport.body_part}`
                : 'Radyoloji PDF’i veya rapor metni eklenmelidir.'
            }
            link="/radiology"
            linkLabel="Radyolojiyi aç"
          />
        </div>
      </section>

      <p className="text-center text-xs leading-6 text-slate-500">
        ℹ️ MediCore hekim kararını desteklemek için geliştirilmiştir; doktorun yerini almaz.
      </p>
    </div>
  );
}
