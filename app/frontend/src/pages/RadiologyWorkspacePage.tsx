...
const REVIEW_CACHE_VERSION = 'v4-python-brain';
...
const LAB_DISPLAY_PRIORITIES: Array<[string[], number]> = [
  ...
  [
    ['nötrofil', 'notrofil', 'neutrophil', 'neu', 'parçalı mutlak', 'parcali mutlak'],
    110,
  ],
...
function reportSummary(report: RadiologyReport | null) {
  ...
  const looksLikeStatistics =
    /\b\d+\s+(klinik bulgu|bulgu cümlesi|ölçüm|kritik uyarı)/i.test(
      technicalSummary,
    );
...
function reportFingerprint(report: RadiologyReport) {
  ...
  const summary = fold(
    [report.summary, report.impression].filter(Boolean).join(' '),
  );
...
export default function RadiologyWorkspacePage() {
  ...
  const [clinicalContext, setClinicalContext] =
    useState<ClinicalIntakeInput | null>(null);
  ...
  const [compatibility, setCompatibility] =
    useState<ClinicalCompatibilityScore | null>(null);
  ...
  useEffect(() => {
    ...
    async function hydrateStoredCase() {
      ...
      try {
        ...
        if (!analysisRunId) {
          setStatus(
            'Görüntüleme kaydı yüklendi; bu hasta için laboratuvar analizi bulunamadı.',
          );
          return;
        }
        if (!context) {
          setStatus(
            'Kan ve radyoloji verileri yüklendi; klinik hasta bağlamı bulunamadı.',
          );
          return;
        }
        if (latestLabResults.length === 0) {
          setStatus(
            'Klinik ve radyoloji verileri yüklendi; yapılandırılmış kan sonucu bulunamadı.',
          );
          return;
        }

        setStatus(
          'Klinik, laboratuvar ve görüntüleme verileri yüklendi; AI değerlendirmesi hazırlanıyor…',
        );
        try {
        ...
        if (!cancelled) {
          setStatus(
            'Kayıtlı kan sonuçları, klinik hasta bilgileri ve benzersiz görüntüleme raporları birlikte yüklendi.',
          );
        }
      } catch (loadError) {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const relevantReports =
      reports.length > 0 ? reports : result ? [result] : [];
    ...
    async function loadCompatibility() {
      try {
        ...
        const brain = await evaluateClinicalBrain({
          ...
          radiology_reports: relevantReports,
          language: 'tr',
        });
        if (!cancelled) setCompatibility(brain.compatibility);
      } catch (brainError) {
        if (!cancelled) {
          ...
          setCompatibilityError(
            brainError instanceof Error
              ? brainError.message
              : 'Python Clinical Brain uyum skoru oluşturulamadı.',
          );
        }
      } finally {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!analysisRunId) {
        setLabResults([]);
        setStatus(
          'Rapor kaydedildi. Kan sonuçları eklenince birleşik değerlendirme oluşturulabilir.',
        );
        return;
      }
      ...
      setStatus(
        'Kan sonuçları, klinik hasta bilgileri ve benzersiz görüntüleme raporları birlikte değerlendirildi.',
      );
    } catch (submitError) {
      setError(
  ...
  const compatibilityClass = !compatibility
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
          Kan sonuçlarını, klinik hasta bilgilerini ve benzersiz görüntüleme
          raporlarını aynı sayfada değerlendirir. Klinik uyum kuralları Python
          Clinical Brain üzerinde çalışır.
        </p>
      </header>
      ...
      {result ? (
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-2">
                ...
                <div className="space-y-4 text-sm text-slate-700">
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <p>
                      ...
                      {[patient?.age, formatSex(patient?.sex)]
                        .filter(Boolean)
                        .join(' / ') || 'Belirtilmedi'}
                    </p>
                  </div>
                  ...
                  <p>
                    <span className="font-semibold text-slate-950">Klinik öykü:</span>{' '}
            ...
            <SectionCard
              title="Kan sonuçları"
              description="Vakayla en ilişkili anormal sonuçlar önce gösterilir."
            >
          ...
          <SectionCard
            title="Benzersiz görüntüleme raporları"
            description="Aynı içerikli mükerrer kayıtlar tek rapor olarak gösterilir."
          >
            {reports.length > 0 ? (
          ...
          <SectionCard
            ...
            description="Tanı olasılığı değil; yapılandırılmış bulguların belirli bir klinik hipotezle deterministik uyumudur. Hesap Python backend'de yapılır."
          >
            {compatibilityLoading ? (
              <p className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                Python Clinical Brain uyum skoru hazırlanıyor…
              </p>
            ) : compatibilityError ? (
              ...
              <p className="text-sm text-slate-600">
                Klinik uyum skoru henüz oluşturulamadı.
              </p>
            )}
          </SectionCard>
          ...
          <SectionCard
            ...
            description="Kan, klinik bilgi ve benzersiz raporlar birlikte değerlendirilir."
          >
            <section className="rounded-xl border border-violet-200 bg-violet-50 p-5">
              <p className="whitespace-pre-line text-sm leading-7 text-violet-950">
                {combinedReviewText ||
                  'Birleşik klinik değerlendirme için desteklenen AI özeti oluşturulamadı.'}
              </p>
