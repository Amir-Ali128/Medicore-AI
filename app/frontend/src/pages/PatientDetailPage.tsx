...
function readPatientProfileFromStorage(): PatientProfile {
  const name = localStorage
    .getItem(LAST_PATIENT_DISPLAY_NAME_KEY)
    ?.trim();

  const age = localStorage.getItem(LAST_PATIENT_AGE_KEY)?.trim();
  const sex = localStorage.getItem(LAST_PATIENT_SEX_KEY)?.trim();

  const birthDate = localStorage
    .getItem(LAST_PATIENT_BIRTH_DATE_KEY)
    ?.trim();

  const hasPdfMetadata = Boolean(name || age || sex || birthDate);
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        ...
        <SectionCard
          ...
          description="Backend-connected navigation for this patient workspace."
        >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <SectionCard
          ...
          description="Latest backend workflow events for this patient."
        >
                ...
                >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    <div className="rounded-lg border border-slate-200 bg-white p-3">
                      ...
                      <span
                        className={`mt-2 inline-flex whitespace-nowrap rounded-lg border px-2.5 py-1 text-xs font-semibold ${severityClassName(
                          hypothesis.severity,
                        )}`}
                      >
