...
export default function PatientDetailPage() {
  ...
  if (isLoading) {
    return (
      <LoadingState
        ...
        description="Fetching backend lab results, clinical prompts, and doctor review status."
      />
    );
  }
  ...
  const reportStatusItems = [
    ...
    {
      label: 'Clinical hypothesis status',
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.85fr]">
        <SectionCard
          ...
          description="Backend clinical hypotheses linked to this analysis run."
        >
          {hypotheses.length === 0 ? (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-lg border border-cyan-100 bg-cyan-50 p-4">
            <p className="text-sm leading-6 text-slate-700">
              Clinical prompts remain under physician review controls.
            </p>
          </div>
