"""Independent Gemini multimodal third reader for radiology uploads.

The adapter receives the original upload directly and never receives Claude or
OpenAI prose. This keeps provider reads independent enough for deterministic
comparison. Output is assistive, non-diagnostic and physician-review-only.
"""
...
async def review_radiology_media_gemini(
    *,
    content: bytes,
    media_type: str,
    modality: str,
    ...
    """Review one radiology upload independently with Gemini vision."""
    ...
    prompt = f"""
Sen MediCore-AI içinde BAĞIMSIZ üçüncü okuyucusun. Claude veya OpenAI yorumunu görmüyorsun;
yalnızca bu yüklenen görüntüyü değerlendiriyorsun. Çıktın hekim incelemesine yardımcı olur,
kesin tanı değildir ve kalibre edilmiş radyoloji anomaly-detector olasılığı gibi sunulmamalıdır.
...
- Başka modellerle anlaşma/uyuşmazlık hakkında yorum yapma; karşılaştırmayı MediCore yapacak.
...
""".strip()
    ...
    text = _clean(getattr(interaction, "output_text", ""), 30000)
    if not text:
        raise ValueError("Gemini görüntü modeli metin yanıtı döndürmedi.")
    ...
    if not summary:
        summary = "Gemini üçüncü okuyucu incelemesi tamamlandı; hekim doğrulaması gereklidir."
